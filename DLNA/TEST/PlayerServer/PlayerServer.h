...
int DeviceHandleSubscriptionRequest(struct Upnp_Subscription_Request *sr_event) {
    unsigned int i = 0;
    int cmp1 = 0;
    int cmp2 = 0;
    ...
    l_udn = sr_event->UDN;
    l_sid = sr_event->Sid;
    for (i = 0; i < PLAYER_SERVICE_NUM; ++i) {
        cmp1 = strcmp(l_udn, player_service_table[i].UDN);
        cmp2 = strcmp(l_serviceId, player_service_table[i].ServiceId);
        if (cmp1 == 0 && cmp2 == 0) {
            UpnpAcceptSubscription(device_handle, 
                                                        l_udn, 
                                                        l_serviceId, 
                                                        (const char **)player_service_table[i].VariableName, 
                                                        (const char **)player_service_table[i].VariableStrVal, 
                                                        player_service_table[i]. VariableCount, 
                                                        l_sid);
        }
    }
...
int DeviceHandleActionRequest(struct Upnp_Action_Request *ca_event) {
    ...
    /*Judge service type*/
    if (strcmp(devUDN, player_service_table[PLAYER_SERVICE_CONTROL].UDN) == 0 && 
		strcmp(serviceID, player_service_table[PLAYER_SERVICE_CONTROL].ServiceId) == 0) {
...
int PlayerCallbackFunc(Upnp_EventType EventType, void *Event, void *Cookie) {
    ...
    case UPNP_EVENT_SUBSCRIPTION_REQUEST:
        DeviceHandleSubscriptionRequest((struct Upnp_Subscription_Request*)Event);
        break;
...
int ReceiveAudioURL(IXML_Document * in, IXML_Document ** out, const char **errorString) {
    ...
    UpnpNotify(device_handle, 
		player_service_table[PLAYER_SERVICE_CONTROL].UDN, 
		player_service_table[PLAYER_SERVICE_CONTROL].ServiceId, 
		(const char **)&player_service_table[PLAYER_SERVICE_CONTROL].VariableName[PLAYER_CONTROL_AUDIOURL], 
		(const char **)&player_service_table[PLAYER_SERVICE_CONTROL].VariableStrVal[PLAYER_CONTROL_AUDIOURL], 1);
	ithread_mutex_unlock(&PlayerDevMutex);
	
	printf("UDN = %s\n", player_service_table[PLAYER_SERVICE_CONTROL].UDN);
	printf("ServiceId = %s\n", player_service_table[PLAYER_SERVICE_CONTROL].ServiceId);
	printf("VariableName[PLAYER_CONTROL_AUDIOURL] = %s\n", 
		player_service_table[PLAYER_SERVICE_CONTROL].VariableName[PLAYER_CONTROL_AUDIOURL]);
	printf("VariableStrVal[PLAYER_CONTROL_AUDIOURL] = %s\n", 
		player_service_table[PLAYER_SERVICE_CONTROL].VariableStrVal[PLAYER_CONTROL_AUDIOURL]);

	/*Create response xml to CP(response xml, action name, service type, arg name, response value)*/
	if (UpnpAddToActionResponse(out, "ReceiveAudioURL", service_type[PLAYER_SERVICE_CONTROL], "NewAudioURL", value) != UPNP_E_SUCCESS) {
		(*out) = NULL;
    ...
    switch(service_type){
        case PLAYER_SERVICE_CONTROL:
            ...
            for (i = 0; i != out->VariableCount; ++i) {
                ...
                strcpy(out->VariableStrVal[i], Player_AudioURL[i]);
            }
            break;
        default:
